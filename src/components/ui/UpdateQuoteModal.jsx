...
import { useGetQuoteByOpportunityIdQuery, useUpdateQuoteMutation as useUpdateQuoteRecordMutation } from '../../services/quote';
import { formatPrice } from '../../utils/FormatValue';
import { toast } from 'react-toastify';

export default function UpdateQuoteModal({ isOpen = false, onClose = () => {}, opportunity = null }) {
    ...
    const [globalMode, setGlobalMode] = useState('suggest');
    ...
    const [updateQuoteRecord] = useUpdateQuoteRecordMutation();
    ...
    // Sync note and quoteId from quoteData
    useEffect(() => {
        if (quoteData) {
    ...
    }, [isOpen, opportunity, oppServicesData, reloadCounter, allServices]);
    ...
    useEffect(() => {
        if (!globalMode || globalMode === 'custom') return;
    ...
    async function handleSubmitQuote() {
        if (!opportunity || !opportunity.id) return setSubmitError('Không có cơ hội để cập nhật báo giá');
        ...
        setSubmitError(null);
        try {
            // Only include existing services (not new ones) in the update
            const servicesPayload = rows
                .map((r, idx) => ({ row: r, originalIdx: idx }))
                .filter(({ row }) => {
                    const isValid = !row.isNew && 
                                    row.id !== null && 
                                    row.id !== undefined && 
                ...
                .map(({ row, originalIdx }) => {
                    const price = globalMode === 'custom' 
                        ? (customPrices[originalIdx] || chosen[originalIdx] || 0)
                        : (Number(chosen[originalIdx]) || 0);
                    
                    return {
                        opportunityService_id: Number(row.id),
                        proposed_price: Number(price),
                    };
                });

            console.log('Services to update:', servicesPayload);

            if (servicesPayload.length === 0) {
                toast.error('Không có dịch vụ nào để cập nhật báo giá. Vui lòng lưu thay đổi dịch vụ trước.');
                setSubmitting(false);
                return;
            }
            ...
            console.log('Sending update quote with body:', body);

            await updateQuote({ id: opportunity.id, body }).unwrap();
            
            // Update quote record note if quoteId exists
            if (quoteId) {
            ...
            toast.error('Cập nhật báo giá thất bại');
            setSubmitting(false);
        }
    }
    ...
    async function handleSaveChanges() {
        ...
        const validServices = rows.filter(r => 
            (r.isNew && r.serviceId) || (!r.isNew && r.serviceId)
        );
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={handleClose}>
            <div className="bg-white p-6 rounded-lg w-11/12 max-w-3xl shadow-lg" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-4 gap-4 text-left">
                    <div>
                        <h3 className="text-lg font-medium">Sửa báo giá {opportunity ? `${opportunity.name || opportunity.title || ('#'+opportunity.id)}` : ''}</h3>
                    </div>
                    <div className="flex gap-2">
                        ...
                        >
                            {isEditMode ? 'Lưu' : 'Chỉnh sửa dịch vụ'}
                        </button>
                        {isEditMode && (
                            <button 
                                onClick={() => {
                        ...
                        <button onClick={handleClose} className="text-xs px-3 py-1 bg-gray-100 rounded">Đóng</button>
                    </div>
                </div>
                    ...
                    {loading ? (
                        ...
                        <div className="mt-4 flex items-center justify-end gap-3">
                            <button onClick={handleSubmitQuote} disabled={submitting} className={`px-3 py-1 text-sm rounded ${submitting ? 'bg-gray-300' : 'bg-blue-600 text-white'}`}>
                                {submitting ? 'Đang cập nhật...' : 'Cập nhật báo giá'}
                            </button>
                        </div>
                        </>
                    )}
