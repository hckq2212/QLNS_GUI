...
export default function ServiceJobList() {
  ...
  const getServiceName = (svcId) => {
    if (!svcId) return '—';
    const s = (servicesData || []).find((x) => String(x.id) === String(svcId) || String(x.service_id) === String(svcId));
    return s ? (s.name || s.service_name || `#${s.id}`) : `#${svcId}`;
  };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold">Danh sách Service Job</h2>
        <Link to="/service-job/create" className="px-3 py-1 rounded bg-blue-600 text-white text-sm">Tạo mới</Link>
      </div>

      {rows.length === 0 ? (
        <div className="text-sm text-gray-600">Không có service jobs</div>
      ) : (
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 align-top">{getServiceName(r.service_id || r.service)}</td>
                  ...
                  <td className="px-4 py-3 align-top">
                    <div className="flex gap-2">
                      ...
                      <Link to={`/service-job/${r.id}/edit`} className="px-2 py-1 rounded bg-yellow-600 text-white text-xs">Sửa</Link>
